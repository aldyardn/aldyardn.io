...
if(empty($nama)){
  header("location:home.php?halaman=volly-add&info=nama-kosong");
}elseif(empty($kelas)){
    header("location:home.php?halaman=volly-add&info=kelas-kosong"); 
// }elseif(empty($jk)){
//     header("location:index.php?halaman=futsal-add&info=jeniskelamin-kosong");
}elseif(empty($alamat)){
    header("location:home.php?halaman=volly-add&info=alamat-kosong"); 
}else{
$add = $koneksi->query("INSERT INTO volly VALUES('','$nama','$kelas','$jk','$alamat')");
echo "<script>
...
<div class="col-md-8">
    <form action="" method="post" class="m-4">
       ...
       <div class="mb-3">
        <button type="submit" class="btn btn-warning" name="volly-add">Kirim</button>
