...
if(empty($nama)){
  header("location:home.php?halaman=basket-add&info=nama-kosong");
}elseif(empty($kelas)){
    header("location:home.php?halaman=basket-add&info=kelas-kosong"); 
// }elseif(empty($jk)){
//     header("location:index.php?halaman=futsal-add&info=jeniskelamin-kosong");
}elseif(empty($alamat)){
    header("location:home.php?halaman=basket-add&info=alamat-kosong"); 
}else{
$add = $koneksi->query("INSERT INTO basket VALUES('','$nama','$kelas','$jk','$alamat')");
echo "<script>
...
<div class="text-center text-hitam bg bg-white m-4">
    <h3><b>Add Anggota Basket</b> </h3>
</div>
<div class="col-md-8">
    <form action="" method="post" class="m-4">
        <div class="mb-3">
            <label for="" class="form-label fw-normal"><b>Nama Siswa</b></label>
        ...
        <div class="mb-3">
            ...
            <input type="text rounded-0" class="form-control"name="alamat">
            <?php if (isset($_GET['info'])) :?>
       ...
       <div class="mb-3">
        <button type="submit" class="btn btn-warning" name="basket-add">Kirim</button>
